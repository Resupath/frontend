...
import { createChat, listChats } from "@/src/types/chat";
import { pipe } from "fp-ts/function";
import { getRoom, RoomWithCharacter } from "@/src/types/room";
...
export default function ChatRoom({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [roomInfo, setRoomInfo] = useState<RoomWithCharacter | null>(null);
    ...
    const asyncListChats = (roomId: string) =>
        pipe(
            listChats(roomId),
            TE.map((response) => setChats(response.data)),
    ...
    const asyncGetRoom = (roomId: string) =>
        pipe(
            getRoom(roomId),
            TE.map((response) => setRoomInfo(response.data)),
            TE.mapLeft((error) => console.error(error))
        )();

    useEffect(() => {
        asyncListChats(roomId);
        asyncGetRoom(roomId);
    }, []);
                    ...
                    {chats.length === 0 && (
                        <div className="flex flex-col gap-2 items-end mt-4">
                            <button
                                ...
                                className="w-full max-w-md p-2 text-left text-sm rounded-lg bg-blue-500 hover:bg-blue-600 text-white transition-colors"
                            >
